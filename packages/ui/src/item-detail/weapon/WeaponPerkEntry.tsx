...
export function WeaponPerkEntry(props: {
  ...
  pending?: boolean;
  /**
   * 这一栏「不符」的直接原因：**本件当前装着它，而来源没要它**（T73 拍板）。
   *
   * 判定条件只有一条：`这一栏 state === "different" && !hit && active`。不加更宽的判定——
   * 「不符」那一栏里按定义**每一张卡都不匹配**（来源侧全都没命中、本件侧全都不在来源名单里），
   * 逐张标＝一屏 9 枚，等于没标。真正值得指出来的只有这一张：它是这一栏为什么不符的答案，
   * 也是换 Perk 时你要动的那个东西。
   */
  mismatch?: boolean;
  /**
