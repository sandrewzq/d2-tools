...
describe("shared vendors page", () => {
  ...
  it("searches service inventory and expands the service accordion", async () => {
    ...
    await user.clear(screen.getByRole("searchbox", { name: "搜索全部商人库存" }));
  ...
  it("keeps inventory visible while showing a partial vendor-detail failure", async () => {
    ...
    await user.type(screen.getByRole("searchbox", { name: "搜索全部商人库存" }), "鹰月");
    expect(screen.getByRole("heading", { name: "仄 · 部分详情失败" })).toBeTruthy();
  });
