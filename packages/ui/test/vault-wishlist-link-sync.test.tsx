...
describe("从链接同步愿望单", () => {
  it("弹框里读完链接，页内预览与命名确认，确认后才落库", async () => {
    ...
    // 读完先收弹框：预览与命名确认在页内那张卡上，与选本地文件之后一模一样。
    await waitFor(() => expect(screen.queryByRole("dialog", { name: "从链接同步愿望单" })).toBeNull());
    expect(store.readLinks).toEqual([linkUrl]);
    expect(screen.getByText("DIMLGpigWeaponWishlist by moc.txt")).toBeTruthy();
    expect(screen.getByText(`来自链接：${finalUrl}`)).toBeTruthy();
    // 提示里给的是校验后的可导入条数之外的整份口径：48 条规则、42 条真正会写进去。
    expect(screen.getByText("已从链接读取 48 条愿望单规则；确认名字后选择新建或覆盖。")).toBeTruthy();
    expect(screen.getByText("42 条可导入")).toBeTruthy();

    // 默认名 = 链接末段文件名去扩展名，用户还可以改。
    const nameField = screen.getByLabelText("推荐来源名");
    expect(nameField).toHaveValue("DIMLGpigWeaponWishlist by moc");
    expect(screen.getByText("「DIMLGpigWeaponWishlist by moc」还没有来源，可以新建。")).toBeTruthy();
  ...
  it("读取失败时弹框不关，错误照原样给出，现有来源不动", async () => {
    const user = userEvent.setup();
    const store = createStore();
  ...
  it("导入文本文件走的是同一张预览卡：展开冗余一样分开说、一样不上警示色", async () => {
    const user = userEvent.setup();
    ...
    expect(await screen.findByText("5 行是展开写法的冗余")).toBeTruthy();
    const notice = screen.getByText(/已识别 42 条可导入的愿望单规则；5 行是展开写法的冗余/);
    expect(notice.getAttribute("data-status")).toBe("success");
    expect(document.querySelector(".vault-knowledge-import-issues")).toBeNull();
  });
});
...
function createSource(label: string, sourceUrl?: string, ruleCount = 42): VaultRecommendationManagedSource {
  return {
