...
export function createAiLightggSource(config: AiLightggConfig): CommunityPerkSource {
  ...
  return {
    name: "AI light.gg",
    isAvailable: () => Boolean(ai?.provider && ai?.api_key && ai?.model && ai?.enable_lightgg),
...
function parsePerkRef(
  ...
  if (typeof perk === "string") {
    ...
    return matched ? { hash: matched.hash, name: matched.name, icon: matched.icon } : { hash: 0, name };
  }
  ...
  if (perk.hash && plugMap.has(perk.hash)) {
    ...
    return { hash: perk.hash, name: matched?.name ?? name ?? String(perk.hash), icon: matched?.icon };
  }
  const matched = name ? findPlugByName(name, plugMap) : null;
  return matched ? { hash: matched.hash, name: matched.name, icon: matched.icon } : { hash: perk.hash ?? 0, name: name ?? String(perk.hash ?? 0) };
}
...
function buildPlugMap(item_hash: number, options: SourceOptions): Map<number, ItemPlugSummary> {
  ...
  const groups = summarizeItemPerks(weaponDef, options.itemDefinitions, {
