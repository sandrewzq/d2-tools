...
//     所以必须整体重算，而不是等下一次盖号。
// 25：来源对外名改为用户给这次导入起的名字（文档标题），文件里声明的名字降为副标题。缓存里
//     存着来源名，不重算的话旧名字会一直显示在卡片与详情上。
// 26：身份推导改为「家族全展开 + 声明版本优先 + 逐 hash 池子自检」（T56 2026-09-24）。旧缓存
//     存的是旧口径的命中（愿望单没展开、CSV 还带着发布组归约）：有的版本少显示了推荐，有的
//     版本显示的是装不上的推荐。这些旧口径导入现在一律不参与匹配，缓存也必须整体重算。
// 27：愿望单文本的归栏改按**规则声明的版本**判（`dimWishlistDiagnostics` 2026-09-24）。此前按
//     手上这件的版本归栏，家族展开到换过插槽池的版本时，装不进标准栏位的 perk 会被丢掉，
//     同一条规则在愿望单里比在推荐表里少一栏，「命中几栏」的两个来源分档因此永远对不上。
const matchAlgorithmVersion = 27;
