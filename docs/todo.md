...
| T8 | P1 | 🟠 静态原型重建中 | 全应用 UI 还原 | [全应用 UI 还原](work/backlog/T8-full-application-ui-functional-convergence.md) | 已完成全局边界、文字 `Tone + Priority`、对比度、图标、Control 状态、密度、层级和响应式合同；共享 Shell、首页与设置页已验收。账号页已收口为“账号数据 Tab + 角色上下文 + 装备位置对照”工作区。资料库页已收口为“Manifest 摘要 + 查询侧栏 + 连续结果列表”工作区，并支持按已装配 Perk 反查关联武器，缺少实例时明确保留定义关联而不伪造实例数据。配装页已收口角色 `ContextSwitcher`、游戏内/本地方案 Tab、状态带、目录/详情/核对工作区和装备定位状态；本地方案的执行清单已固定为 3 武器 + 5 护甲，非装备收藏项退出应用路径，职业分支 / 星象 / 碎片 / 技能转为独立构筑记录并明确 Plug 验证前不可切换。商人页已收口为真实地点目录、库存章节、角色上下文与核对栏；Offer 使用真实图标、成本、数量、可购买/受阻状态，非装备库存不再伪造详情跳转。仓库页已收口为四个语义 Tab、筛选实例列表、基于玩家标签的清理空态、Hash 同名对比和推荐来源空态；非武器/护甲实例不再伪造详情跳转。三原型已完成根节点、详情骨架、Overlay、控件/导航/响应式合同迁移，详情原型共用抽屉关闭模型，所有原型 CSS 的可见字体下限均为 `12px`。下一步审计三个冻结原型的跨页一致性，再将已验收的静态原型迁入共享 UI。 |
