...
| T8 | P1 | 🟠 静态原型重建中 | 全应用 UI 还原 | [全应用 UI 还原](work/backlog/T8-full-application-ui-functional-convergence.md) | 已完成全局边界、文字 `Tone + Priority`、对比度、图标、Control 状态、密度、层级和响应式合同；共享 Shell、首页与设置页已验收。账号页已收口为“账号数据 Tab + 角色上下文 + 装备位置对照”工作区。资料库页已收口为“Manifest 摘要 + 查询侧栏 + 连续结果列表”工作区，并支持按已装配 Perk 反查关联武器，缺少实例时明确保留定义关联而不伪造实例数据。配装页已收口角色 `ContextSwitcher`、游戏内/本地方案 Tab、状态带、目录/详情/核对工作区和装备定位状态；游戏内与本地模式均已在浏览器中复核。已修复 AI 任务栏移除和资料库历史锚点变化后初始化脚本仍访问旧节点，导致所有页面内容未激活的回归。三原型已完成根节点、详情骨架、Overlay、控件/导航/响应式合同迁移，详情原型共用抽屉关闭模型，所有原型 CSS 的可见字体下限均为 `12px`。下一步逐菜单收敛商人和仓库，再将已验收的静态原型迁入共享 UI。 |
