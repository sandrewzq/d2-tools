...
| 编号 | 优先级 | 状态 | 问题 | 下一步 |
|---|---|---|---|---|
| Bug #2 | P1 | 🟡 待验证 | 切换武器 Perk 使用过时请求结构、错误角色或旧实例详情，导致 ErrorCode 25、1623、1679，并在成功后继续显示旧配置 | 实例详情刷新已改为绕过服务层与底层 Bungie 请求缓存；写入前、1679 后重试、写入成功后均读取服务端最新实例。Perk 与锁定、装备、转移等详情操作现就地显示提交、刷新、成功和失败恢复状态，不再把反馈写到账号页；由真实武器验证 |
