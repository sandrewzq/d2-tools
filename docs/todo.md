...
原型视觉复查已收口：375px 顶部状态改为可展开入口，首页刷新信息消除横向裁切，AI 窄屏抽屉接管完整工作区宽度，折叠菜单保留无障碍名称；三份原型默认画布不再混入验收控件，追加 `?controls=1` 才显示原型状态控制器；护甲定义无固定属性时改为紧凑提示，获取来源占满阅读宽度。下一步按该冻结基准迁入共享 UI。
