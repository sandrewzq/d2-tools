...
| T8 | P1 | 🟡 待 CI | 资料库自动更新与重试机制收口 | - | 已统一更新/修复单任务编排并拆除双层重试，补齐请求超时、4 路有限并发、组件进度、磁盘预检、语言立即检查、跨天检查、损坏缓存降级、版本/语言/组件完整性、轻量状态侧车、staging/backup 恢复清理和失败状态反馈；本地完整 `pnpm test` 已通过，后续仍需设计运行中取消、跨应用实例文件锁与更深记录级校验，等待普通 push 后由 CI 验证 |
