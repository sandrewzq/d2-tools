...
| T8 | P1 | 🟡 原型已完成，待迁入共享 UI | 全应用 UI 还原 | [全应用 UI 还原](work/backlog/T8-full-application-ui-functional-convergence.md) | 三份 HTML 原型已完成全局边界、文字 `Tone + Priority`、对比度、图标、Control 状态、密度、层级和响应式合同迁移。全应用原型已完成首页、账号、仓库、配装、资料库、商人和设置工作区；仓库筛选已恢复为武器、护甲、全部三种单选范围，武器与护甲领域条件独立渲染，冻结快照缺少的弹药、框架和六维属性明确禁用而不伪造数据。配装仅将 3 武器 + 5 护甲纳入应用计划，构筑配置在 Plug 验证前仅记录。武器/护甲详情已共用根、状态层、抽屉、遮罩和焦点模型：初始状态与每次切换均由详情根节点唯一控制，正常态覆盖层隐藏，最终文字字号与颜色由 DetailDossier 共享 Typography 层统一；护甲实例属性的最小文字已收口到 12px。下一步按已冻结的三份 HTML 迁入 `packages/ui`，保留真实数据、行为、加载/空/失败与恢复能力。 |
