...
- 仓库推荐运行时只维护 `Map<instanceId, VaultItemInstanceMatchInfo>`，同名版本共享推荐但每个实例使用自身实际 Roll 独立计算。人工来源六项优先级为 `Perk 1 / Perk 2 > 第一列 / 第二列 > 大师 / 起源特性`，主结论固定使用“全部符合 / 核心符合 / 接近推荐 / 关键缺失 / 仅推荐这把武器 / 数据不完整”，`x/y` 只作完整来源事实；DIM 保持无序且全部必需的原生 Hash 组合。匹配结果按 Roll 指纹、Manifest 版本、人工知识库 revision、DIM revision 和算法版本缓存；位置、光等、锁定和玩家标签不使 Roll 分析失效。卡片、推荐筛选、账号摘要和同名整理复用轻量实例摘要，禁止在单卡渲染中遍历完整 Wishlist；完整可掉落池只在详情中由玩家展开后按 Manifest 版本加载。批量操作必须跳过锁定、精确配装引用、手动保留、独特 Roll、来源冲突、数据不完整和未覆盖武器，应用永不自动锁定、解锁或分解。
