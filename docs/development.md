...
6. 明暗主题必须使用同一套语义 selector，只替换 token；颜色模式由 `config.json` 的 `features.color_mode` 持久化。**真源只有这一个**，DOM 上分两处投影：`<html data-theme>` 管壳外（启动回退屏、`body` 底色），`.app-shell[data-color-mode]` 管壳内。壳外那份由 preload 从主进程下发的 `--d2-color-mode` 赶在第一帧之前写上——启动回退屏挂在 `#root` 下、在 `.app-shell` 之外，不写就会在亮色模式下吃 `:root` 的深色值，看着像黑屏；设置页改主题时 `AppShell` 的 effect 两处一起写，不许只改一处。窗口用 `show: false` + `ready-to-show` 再显示（`did-fail-load` 兜底），页面渲染出内容之前窗口不存在（T75）。
...
9. 同一个视觉对象只保留一个实现组件，调用方只传状态与文案，不复制外观。历史包袱里同一页出现两套长相不同的同类卡片（如武器详情的 Perk 条目，T69 已统一为 `WeaponPerkEntry`）时，保留一套、删除另一套，而不是加 `!important` 或更高 specificity 去对齐；条目的宽度约束属于承载它的列表，条目自身不设宽度。加载骨架与空态占位也走同一个条目盒子（同一条盒子类、同一处几何），页面不得为占位另写一套 min-height / padding / 图标形状。这类不变量由 `test:architecture` 守卫，不靠人眼复查。
10. 详情的「加载态」只由正片读取决定：打开详情、点按需按钮触发的完整读取、显式刷新。后台补读（只为某一块补数据，如武器固有能力只补定义）不得置整份详情的加载态——置了宿主会把整份详情换回全屏骨架，表现为「第一次打开就白屏」，而骨架重挂载又会再次触发同一个补读，读取失败时就成了「一直闪」。补读必须单飞、失败只报一次且不自动重试（重试入口是用户点得到的按钮），不读自己那一块以外的数据（补定义不读完整实例 Roll）。
**上面那句管的是「谁可以置位」，还有一句管「置了位之后视图层拿它做什么」：加载态是「忙碌」，不是「能不能渲染」。** 视图层不得拿它当内容闸门，去撤销一份已经画出来的正文。落点是宿主的就绪判断：它读一条**单调闩锁**（只在「属这件装备的内容第一次可用」时合上，之后只往前走），不与实时的忙碌标志做合取；忙碌只走 `isBusy`。于是**写后读回**（应用 Perk 之后后台连读若干次确认服务器状态）和**已经有了内容之后的任何重新读取**都算后台读取：正文留在原地、由新数据就地替换，不换回全屏骨架——换回去会顺带丢掉尚未应用的改动、滚动位置与所在章节。写后读回自 T77 起本来就走 probe 模式（不置加载态），这条闸门的两个触发源因此都不该再让正文闪一下。
这条由 `test:architecture` 守卫两处：`loadSelectedItemDefinition` 的正文与接线（不许置加载态），以及宿主的就绪闸门（必须是闩锁、不许接回实时忙碌标志）。

配装页明确区分 Bungie 官方 `游戏内配装` 与本应用保存的 `应用配装`：两者保持独立一级视图，通过“复制到应用配装”和“保存到游戏内槽位”显式转换。应用配装承载方案库、对比、槽位式创建/编辑与安全穿戴；逐部位护甲求解只作为编辑器护甲区的“按属性目标自动配甲”能力。旧 `LoadoutTemplate` 兼容页不再保留。新建配装的入口按玩家的四种动机组织（存下现在这一身 / 抄一套 / 凑一套 / 改一套），四条路共用同一条主干：定来源 → 装武器 → 定子职业 → 调护甲 → 挑候选 → 核对能不能穿 → 落盘，差别只在第一步带进来什么、哪几步是空的。改一套的出口有两个：存回原方案或另存为新配装；游戏内槽位那一侧没有对应能力，Bungie 的槽位只能整体覆盖。
...
- 武器详情保持单 Overlay 原子 Loading / Ready 状态机；章节顺序固定为 推荐 Roll → 当前配置（本件 Roll）→ 完整掉落池 → 属性与获取 → 升级与锻造，完整掉落池是本件 Roll 之后独立成节的区域，不是配置章节内部的一块。首屏挂载身份、推荐 Roll、当前配置、本件 Roll、完整掉落池按钮和实例操作；推荐 Roll 在最上方，所以推荐证据在打开详情时读取，不再等玩家滚到附近。属性与获取、升级、AI 章节由 `IntersectionObserver` 在玩家访问或接近视口时挂载并保留，章节导航不得在滚动事件中逐项同步读取布局。只有身份主图 eager，其余水印、Perk、来源和升级图标 lazy；属性与获取章节激活后才并行读取同名版本和实时获取状态。推荐 revision 在详情打开期间变化时，未访问章节只重装按需 loader，已访问章节只重载推荐区，旧会话结果不得覆盖当前对象。配置列判定「哪些项可切换」必须分开插槽状态是否核实过：`detail_loaded.instance` 为真（`sockets` 是游戏返回的）才用 `is_enabled` / `can_insert` 等硬条件；未核实时 `sockets` 来自 `buildPreviewSocketsFromWeaponRoll`，那几项是快照占位值，`false` 的语义是「快照不知道」而不是「不可用」，此时按快照判「拥有且不是当前这项」。把预览占位当插槽事实读，会让整件武器在读完完整 Roll 之前一律不可切换，本件 Roll 的格子、推荐区浮层里的「选择」与写面板会一起哑掉（T73 实窗复验）。推荐区按对象身份分流，不靠字段有没有值猜层次：只有 `model.context.kind === "account_instance"` 才画逐栏两列的「来源要求 ｜ 本件拥有」对照，资料库定义与商人 Offer 只列来源要求的栏位与候选——没有本件就没有命中数、完整度、匹配分数和「未选择账号装备」；整份推荐集的免责声明在推荐区顶部说一次，来源自己写的说明逐卡显示（T83）。账号实例的逐栏对照里，卡片自己的事实与这一栏的结论是两条通道：「本件当前装着、来源没要它」由卡片自己判定（`!hit && active`），不受该栏「符合 / 不符」门控，「当前启用」那枚环不参与表达对错、颜色只表示「当前」（T84）。详情页的层级分家：章节页签 > 悬停的 Perk 条目与它的说明浮层 > 区域内吸附栏头 > 静态内容。浮层是从卡片往上开的、必然跨过栏头，所以不透明的吸附构件不许和悬停层同层——同层就会把浮层从中间切成两截（T85）。
...
安装包必须携带项目许可证和桌面运行时第三方许可证清单。`pnpm licenses:generate` 根据当前依赖生成 `packages/desktop/build/LICENSE.txt` 与 `THIRD_PARTY_NOTICES.txt`；质量门禁通过 `pnpm licenses:check` 拦截依赖变化后未更新的清单。NSIS 安装向导展示项目许可证，两份文件同时进入安装资源并可从设置页打开。设置页来源列表中的 GitHub / 在线资料链接交给平台默认浏览器打开，不在应用窗口内导航；本地许可证按钮使用系统默认程序打开安装包内文件。根 `LICENSE` 末尾除 MIT 条款外还带一段 Bungie 游戏内容与商标声明，这段声明按 Bungie Game Content Usage Rules 的要求附在分发包里，`packages/ui/src/startup/StartupGate.tsx` 的首次启动页也显示同一段声明，不要删。
...
- 对用户的最终回答和本次改动里新增或重写的文档，成稿前调用 `shuorenhua` skill 过一遍（真源 `~/.agents/skills/shuorenhua/`）；它只删包装语、客套和翻译腔，数字、`T数字` / `Bug #数字`、`file:line`、术语原名、责任主体和限定语必须原样保留，结构不进入改写。完整口径见 `AGENTS.md` 的「说人话」一节
