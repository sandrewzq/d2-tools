...
- 武器推荐知识库同样遵循“CSV 维护、SQLite 查询、DTO 传输”的边界：应用导出的 UTF-8 BOM、31 列 CSV 模板是唯一导入列合同；用户按模板填写后，由 Desktop 主进程完成文件选择、完整校验、预览确认、内容指纹核对和事务导入，并把确认文件托管为 `<data_dir>/imports/weapon-recommendations.csv`，运行时查询 `<data_dir>/knowledge/weapon-recommendations.sqlite`。`攻略/T20-武器推荐知识库/武器推荐.csv` 仍是仓库人工审核源，也可用 `D2_WEAPON_RECOMMENDATIONS_CSV` 指定开发输入；Renderer 不直接读取 CSV 或 SQLite。第三方再分发许可未确认前公开安装包不得内置 CSV。DIM Wishlist 是独立的第三方交换格式，原生 `.txt` / `.wishlist` 文件直接在主进程解析和预览，不转换为 CSV；分段标题、说明、notes 和 tags 按来源块保存一次并由规则引用，避免 Voltron 大文件重复保存长文本。发布版 DIM 社区推荐通过 GitHub commit API 查询 `voltron.txt` 最新 SHA，再用 Contents API 下载该固定 revision；检查只生成临时预览，玩家确认后才事务激活到 SQLite，并保存上游 URL、commit SHA、ETag、原文件 SHA256、检查时间和激活时间。网络、限流、解析或激活失败均继续使用当前数据，本地文件导入仍是离线回退。统一推荐数据库当前为 schema version 3；`dim-wishlist.json` 与 `local-community-recommendations.json` 只允许在首次读取时事务迁入 SQLite，迁移成功后保留原文件作磁盘回退但不再参与正常查询、保存或清理，迁移失败则不写完成标记且不替换当前数据。
- 仓库推荐运行时只维护 `Map<instanceId, VaultItemInstanceMatchInfo>`，不再生成或回退到武器 Hash 聚合结果。知识库来源按六栏逐项核对；DIM 规则保持无序且全部必需的 Hash 集合，并额外输出唯一归栏、同栏多必需项、跨栏歧义、未知栏位和特殊插槽诊断。资料库未准备、资料库版本落后和推荐知识库不可用通过结构化原因传输；统一 SQLite 仅有 DIM 数据但中文推荐记录为空时仍视为中文知识库不可用。批量待处理必须跳过锁定、精确配装引用、手动保留、独特 Roll、来源冲突、数据不完整和未覆盖武器；任一来源已有确认符合项而另一来源为 `0/y` 时属于来源冲突，且应用永不自动解锁或分解。
