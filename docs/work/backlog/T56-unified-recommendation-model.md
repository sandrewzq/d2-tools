...
> 状态：🟠 进行中
> **判据① 五路对拍 / 判据② 边界扫描：全绿（2026-09-16）。**
> Layer 1 / 2 / 3（含 L3-0～L3-4b）/ 4 / 4.5 / 5 均已完成，S8 v11 与判据复跑已过；**Layer 6 三条已全部闭合（2026-09-16）**。
> **判据① 与判据② 之外已完成**：L3-2c 归约合并、Layer 6 三条、判据① 复验时发现并拍板的**分组粒度分叉**（用户 2026-09-16 定：按来源名分多行），以及同一轮实测撞出并修掉的**「停用整份导入」在 CSV 上失效**，以及**导入期校验（原 T60，2026-09-16 完成，含 4 处变异验证）**。剩余只有 Bug #80–#87 的人工复验（用户自行重启应用后确认）。
> 合并说明：原 T55 / T57 / T59 / T60 / T61 与 Bug #80–#87 已并入本文件，编号冻结作追溯；`docs/todo.md` 只保留 T56 一行。
...
5. **不做临时双写、不做自动迁移**（做法一）：读取 / 写入 / 状态门 / 管理面 / 导出同一次落地，落地后 **CSV 需用户重新导入一次**（DIM 侧不重导）。这条对外代价**尚未告知用户**。
...
- **`knowledge_metadata` 表本身不删**——（2026-09-17 更正：它现在**没有任何读写方**。当初的理由是 `dimWishlistUpdates.ts` 仍用它做在线更新缓存，而那条固定地址在线导入已随 [T63](T63-dim-source-link-sync.md) 删除；表仍保留，因为它是 `hasRecommendationTables` 识别 v1 旧库的标志之一，删表要连识别口径一起改，属于另一次 schema 变更。）
...
**核实结果（2026-09-16，逐条 grep 过）**：`docs/development.md` 的 5 条按类型规则**已全部清零**（`DIM Wishlist` / `dim_wishlist` / 四位作者白名单 / 「固定排在」/ 「DIM revision」全仓 0 处）。但**口径残留不在 development.md，而在另外三份参考文档**，属于「面向用户不出现格式名」这一条的收尾，另立清单跟踪：

| 位置 | 残留 |
|---|---|
| `ui-specs/application-workspaces.md:207` | 整段以「DIM Wishlist 是第三方辅助数据」组织，并把两种格式分开叙述（其中「笔误行按行忽略 / 整把武器跳过」是导入期校验的口径，与本任务〈导入期校验〉一节一致，改文案时不要改掉） |
| `equipment-detail-and-knowledge-analysis.md:448` | 「DIM Wishlist 保留为用户手动导入的个人目标」 |
| `destiny-tool-reference.md:117` | 「来源身份：社区维护的 DIM Wishlist 聚合」——**这条是外部工具的客观描述，不是本应用的用户文案，建议保留** |
| `user-guide.md` §8.3 | 「导入 DIM Wishlist」等切换前名称（已在 `docs/todo.md` 登记） |

**为什么没有顺手改**：这三处是合同 / 参考文档，改它们要同时核对「导入期校验」口径与用户指南的措辞，属于独立的一次文档校正，硬塞进 L3-4 会让「文档口径一致」这条判据无法被单独验证。
...
护甲详情的「目标匹配」区有同样的按硬编码来源身份分页签的结构（`ArmorTargetSource = "personal" | "loadout" | "community"`，其中两个页签全仓无生产点、恒空）。它涉及护甲侧信息架构取舍，**不在 T56 内处理**，已拆到 [T62](T62-armor-detail-target-source-defork.md)（待用户拍板 A / B / C）。
...
## 待复验子项（Bug #80–#87，代码已完成，不阻塞架构工作）

这七项代码改动已落地、类型检查通过，只差重启 Desktop 人工确认。

| 编号 | 内容 | 复验要点 |
|---|---|---|
| Bug #80 | 结果排序下拉被移入命令栏，左侧无法排序 | 重启确认左侧可切换排序、命令栏无重复入口 |
| Bug #81 | 「位置」占左侧筛选栏，命令栏留白 | 确认武器模式可切换、护甲模式不显示 |
| Bug #82 | 筛分区标题冗余、顺序不符合使用习惯 | 确认三种模式顺序与标签齐全 |
| Bug #83 | 武器框架 chips 展开后过长 | 确认只显示一个选项、数量与结果一致 |
| Bug #84 | 槽位与锻造筛选缺图标和颜色区分 | 确认图标、色调与选中态 |
| Bug #85 | 推荐来源页导入导出混排、模板位置错 | 确认分组与焦点行为 |
| Bug #86 | 来源页对齐错位、「复制验收报告」占据首屏 | 确认三处对齐、详情弹框内诊断区、提示文案 |
| Bug #87 | 「复制验收报告」完全归入「设置 → 诊断」 | 需让设置页拿到账号武器集合与扫描状态（跨 provider）；有实际需要时再排期 |
...
| `ambiguous_slot` | 该 Perk 能落在多个栏位，无法唯一归栏 | 诊断状态 `cross_slot_ambiguous` |
| `same_slot` | 同一条规则里两个 Perk 落在同一栏 | 诊断后标准栏位重复 |
...
| `desktop/main/ipc/wishlist.ts` | 文件预览 / 确认都跑校验，**落库写的是过滤后的规则**且与预览同一个函数；定义池装载失败退回空池（当时还有一条粘贴路径的写入同样过一遍，该路径已随 [T63](T63-dim-source-link-sync.md) 删除） |
...
1. ~~**粘贴路径没有服务端预览**：粘贴仍由渲染层本地解析并即时显示…~~ **已随 [T63](T63-dim-source-link-sync.md) 关闭**——粘贴通道整条删除，不再有这条缺口。
2. ~~**在线导入（URL）不走这套校验**：`dimWishlistUpdates` 是从固定地址拉取并直接换库的在线更新流程…~~ **已随 [T63](T63-dim-source-link-sync.md) 关闭**——固定地址在线导入整条删除；取而代之的「贴链接读取」与本地文件共用同一条解析 / 校验 / 预览流水线（`wishlist.ts` 里三个入口走同一个 `parseDimWishlist` + `filterDimWishlistForImport`）。
