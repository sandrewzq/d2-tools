...
> **剩余只有 Bug #80–#87 的人工复验**（用户自行重启应用后确认），以及下方「如实记录」里主动押后的那一项。**代码侧本任务已无欠账。**
...
| ~~`ambiguous_slot`~~ | 已删除：该 Perk 能落在多个栏位时按作者书写的栏位顺序归栏，归栏恒为单值，不再有「归不了栏」这一档 | 见 `Bug102-cross-slot-perk-column-by-write-order.md` |
