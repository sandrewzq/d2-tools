...
- 首个 `guide_capture` 成果已接入：明确的配装、构筑或攻略问题可生成带上下文快照和结果引用的草稿卡，用户点击后进入攻略页并预填未保存正文；保存、要求识别、账号匹配、待确认项和显式保存仍由攻略/配装工作流负责。
...
- 武器匹配职责完成重排：DIM Wishlist 迁入仓库“目标与匹配”的低频操作；历史“本地社区推荐表”仅保留兼容读取并标记为遗留数据，不再提供 Hash / JSON / CSV 普通入口。仓库只突出当前账号的命中装备、组合、模式和匹配依据，并可直接打开命中装备；批量匹配与装备详情统一聚合全部本地数据，AI / light.gg 仍不参与仓库批量扫描。
- T7 核心能力切片此前已通过 `pnpm test` 与 `pnpm typecheck`；本轮入口收敛和 Wishlist 迁移尚未运行本地自动化验证，也未完成真实账号与 Desktop 实窗验收。`guide_extraction_patch` 缺少独立的领域修订命令和人工审阅收益，暂作为后续可选增强搁置；当前确定性提取与逐项人工确认继续作为攻略要求真相，不把 AI 建议混入已确认记录。
...
- T7 不重写当前已经可用的账号、资料库、商人和仓库领域真相；当现有页面直接暴露低频技术配置或不同页面产生不一致结果时，必须调整产品职责和证据 Adapter，不能以“保留现有逻辑”为由跳过用户旅程。
...
当前已实现 `guide_capture`、`armor_solution_comparison` 和 `equipment_target_candidates`。前者根据明确的配装/攻略意图保存本轮问题、回答、`source_snapshot_id` 与底层 `result_ids`，交接后进入攻略页正文草稿；护甲成果直接引用确定性 `armor.plan`，保存目标与候选摘要，并额外记录底层 Armor `result_id`；装备目标成果合并 `account.find-items` 与 `manifest.search-items`，区分真实实例和仅定义候选。`guide_capture` 必须先经过攻略保存和要求确认，后两类成果进入配装页专用审阅，三类成果都不能直接保存或执行。`guide_extraction_patch` 保留为未来可选类型，不属于当前 T7 完成标准。
...
- “推荐数据”改为“目标与匹配”，高频结果优先于低频数据导入。
- 展示当前账号的推荐组合命中、目标命中、DIM、攻略和 Armor Planner 证据，不合并成隐藏总分；来源导入与移除归设置页管理。
...
- 旧任务入口已经保存的文本只在配装页提供一次性兼容恢复，由用户生成草稿或放弃；新攻略内容不得再通过该入口写入。
...
已实现攻略、目标、Armor 方案、配装、结果和操作日志双向追溯。攻略链接、正文和 AI 整理文本统一从攻略页进入；配装页只接收确认后的账号候选、护甲条件和其他类型化成果。DIM Wishlist 迁入仓库“目标与匹配”的低频操作，设置不再承载业务导入；历史自定义规则只保留兼容读取，不再提供 Hash / JSON / CSV 普通入口。导航、正式 UI 合同与用户文档已同步，当前只待本地自动化与真实账号 Desktop 实窗验收。
