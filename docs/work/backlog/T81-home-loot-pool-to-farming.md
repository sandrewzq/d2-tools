...
玩家的决策信息在**本周刷取**面板里：`WeeklyFarmingPanel.tsx` 已在显示推荐 / 图样 / 账号持有 / 决策词。
两边说的是同一批武器，但首页那串名字点不动，也没有「我已经有了 / 这个推荐刷」的标记。

## 二、落点由 T45 定，不是本件新决策

`T45-weekly-farming-checklist.md` 已经写死两条：

- 第 51 行：完整入口放在「账号 → 本周行动 → 本周刷取」；本周刷取依赖当前账号、活动轮换和时间窗口，
  属于需要采取行动的动态清单，不属于资料库的稳定检索内容。
- 第 179 行：从首页摘要、资料库、仓库或商人深链接进入时，**先定位到对应活动或武器**。

所以本件是兑现 T45 已定的深链接口径：跳账号菜单（`AccountPageContentView` 的 `section === "weekly_farming"`），
不跳资料库的 `weekly_farming` 页签。
...
| 消费 | `ui/src/account/AccountPageContentView.tsx` 收到请求后切到 `weekly_farming` 分区，再滚动到 `#library-weekly-<key>` |
...
账号页里活动的 DOM id 由目录资源的 `activity.key` 决定，而 `buildWeeklyFarmingCatalogResource`
对两类活动给的 key 不是一套：
...
- 首页轮换突袭 / 轮换地牢卡片里点「查看本周刷取」，切到账号菜单的「本周行动 → 本周刷取」；
- 页面滚动到对应活动，而不是停在分区顶部；
- 同一条目重复点击仍然重新定位；
