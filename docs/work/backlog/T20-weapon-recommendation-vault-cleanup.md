...
> 更新时间：2026-09-03
> 状态：阶段 0–9 代码主线已完成；同名武器共享推荐、实例匹配与仓库列表性能已收敛，待真实账号和实窗验收
> 优先级：P1
...
- Aegis、LGpig、YXCRALLXY、Sayalarry 均按“一把武器 + 一个来源一行”处理；来源行内枪管、弹匣、大师、Perk 1、Perk 2、起源特性是最多六个独立候选池，同栏多个候选为任选其一。面向玩家统一显示“`x/y 栏符合`”，空白、“任意”和未指定栏位不计入分母。
- DIM Wishlist 保持“一行一个完整 Roll 组合、组合内全部 Perk 必需、多行组合任选其一”的原生语义，不套用人工来源六栏计数，也不跨组合累计 Perk。列表只显示“`DIM：命中 n 组`”或“`DIM：未命中 · 最接近 x/y Perk`”，不显示拇指图标，不在卡片展示可能达到数千的总组合分母。
- 面向玩家不使用“完整命中、部分命中、核心已齐”等概括词。CSV 中经过清洗并确认保留的来源栏位全部进入分母，运行时不得因为当前 `itemHash` 的定义池不包含某个要求而删除该栏位；实例实际拥有即符合，实际没有即不符。只有账号武器 Roll 本身没有完整读取时才显示独立的“Roll 数据异常”，不再产生“n 栏待核对”。
...
- DIM 列表摘要优先显示完整命中组合数；没有完整命中时，只从同一条原始组合计算“最接近 `x/y Perk`”。详情默认最多生成三条组合卡：存在完整命中时只取命中组合，没有完整命中时按符合比例、符合数和要求数选择最接近的组合，避免一次构建数千条详情节点。参考 DIM 的 Perk 标记和 Armory 组合整理思路，但本应用不使用拇指图标，统一使用文字状态和逐项“符合/不符”。如后续需要浏览全部组合，再增加按需读取与虚拟化，不恢复一次性全量渲染。
...
- “资料库未收录”与“来源已有记录但 0/y 栏符合”必须区分；两者都不等于可以分解。
- 受锁定、精确配装引用、玩家手动保留或实例 Roll 数据异常的武器，不得被系统自动改为待处理；后续批量整理只写玩家已经确认的本地状态。
...
- 账号快照通过一次 Profile 请求生成紧凑 `weapon_roll`；SQLite 名称级来源输出六类明确要求，Core 按实例逐来源计算要求总数、符合数和逐栏状态，并通过 IPC/App 实例 Map 传输。仓库筛选卡、“推荐与目标”、同名整理和单件武器详情均消费 `source_matches`。
- DIM 原始 Hash 集合保持“无序且全部必需”语义；应用按当前武器官方 Socket 诊断唯一栏位、同栏多个必需项、跨栏歧义、缺失定义与特殊异域插槽。旧 Hash 只有在官方名称能唯一归属当前插槽时才映射，不猜测第三/第四栏。
- 资料库未准备、资料库版本落后和推荐知识库不可用使用结构化原因传输；统一 SQLite 仅存在 DIM 数据但中文推荐记录为空时仍视为中文知识库不可用，DIM 或本机规则继续保留部分结果。仓库提供只读验收报告，记录扫描完整性、Manifest 版本、推荐库 Schema/指纹、0/y、Roll 数据异常、异域、跨版本同名和 DIM 诊断。
...
- DIM 作者、标签、notes、标题和用途解析已经完成；完整实例 Plug 输入已经接通。UI 已在武器卡、“推荐与目标”、同名整理和单件详情正式消费实例结果；人工知识库来源显示 `x/y 栏符合`，DIM 单独显示“命中 n 组”或“未命中 · 最接近 x/y Perk”，不显示卡片总组合分母，也不使用拇指图标。同名整理可在实例 Roll 与当前组来源要求间切换，单件详情按来源卡固定展示六栏要求、本件拥有、当前启用与文字状态。
...
完成门槛：每个实例都有独立 Roll 指纹；缺少 reusable plugs 或无法读取实际栏位的实例明确标记“Roll 数据异常”，不得预填或改写任何玩家整理状态。推荐按武器身份跨历史、复刻和高阶版本共享，当前 Hash 只作追溯，不作为推荐适用条件。
...
1. `筛选列表`
   ...
   - 仅在武器条件中增加“推荐来源”和“对照结果”筛选。对照结果使用要求全部符合、有部分符合、仅推荐这把武器、资料库未收录、Roll 数据异常等事实筛选，不引入“核心已齐”等术语。
   - 武器卡只显示玩家整理状态、保护事实和最多两条来源摘要。人工来源使用 `Aegis：5/6 栏符合`；DIM 完整命中时使用 `DIM：命中 1 组`，未命中时使用 `DIM：未命中 · 最接近 2/5 Perk`。DIM 不显示拇指图标，不在卡片展示总组合分母，不堆放逐槽位细节，也不从来源摘要推导整理状态。
   - 点击实例在现有详情中定位到“目标匹配”章节；宽屏不把详情永久挤入仓库网格，窄屏复用详情 Drawer 与焦点恢复规则。
2. `同名整理`
   - 保留现有同名组目录、比较基准、当前启用/完整 Roll 模式、待应用整理状态和保护提示。
   - 新增“比较来源”选择；一次只以一个来源展开矩阵，按该来源明确要求的真实插槽比较每个实例的符合/不符；实例 Roll 未读取时显示数据异常。其他来源在实例列底部以独立 `x/y` 摘要显示，不跨来源拼接。
   - 在相同规范武器名称内比较，允许历史、复刻和高阶 `itemHash` 进入同一组；基准只突出差异，不自动决定保留哪一把。
3. `推荐与目标`
   - 内部分为“武器推荐”和“个人目标”两个 `tabpanel`。个人/装备目标、攻略引用和跨页定位完整保留，不再与武器推荐并排压缩。
   - “武器推荐”按实例默认显示全部账号武器，允许按有来源记录、有符合项、来源 `0/y`、来源冲突、Roll 数据异常、无来源记录和有人工标记查看；这些是可重叠的查看条件，不是系统评级。
   - 每行显示来源级 `x/y` 摘要、用途、锁定/配装引用和人工标记，可打开单件详情查看逐来源六栏证据；存在同名副本时可直接切换到对应同名组并恢复目录焦点。
...
4. `单件详情`
   ...
   - 每张人工来源卡显示来源名称、用途、原始评级/说明、原始链接、更新时间和 `x/y 栏符合`；逐行列出有效来源要求、这把实例拥有的值、当前启用值及符合/不符/来源未要求。普通玩家不显示 CSV 版本、Manifest `releases.v*` 或“当前版本是否适用”等生成证据。实例 Roll 未读取时整张来源卡显示“Roll 数据异常”，不显示残缺的 `x/y`。DIM 保持组合卡，不伪装成人工来源六栏记录。
...
5. “复制验收报告”提供真实账号只读抽查材料，覆盖五来源 0/y、Roll 数据异常、异域、跨版本同名、DIM 组合、扫描完整性和版本信息。
...
6. `实现：标准与紧凑武器卡片`：标准模式为默认，图标提升到 `52px`，卡片最小宽度 `220px`；人工来源最多两行并使用“`Aegis：4/6 栏符合`”。DIM 独立使用“`DIM：命中 1 组`”或“`DIM：未命中 · 最接近 2/5 Perk`”，不使用拇指图标，不展示可能达到数千的总组合分母。DIM 的最佳组合在匹配阶段预先计算，优先完整符合，再按符合比例和要求数选择，并随实例匹配结果进入 SQLite 缓存；卡片不重复遍历组合，也不跨组合累加重复 Perk。更多来源显示“另有 N 个来源”，完整 Perk 和组合说明留在详情。
...
- 每个人工来源都能解释为 `x/y 栏符合`、仅推荐这把武器、该来源未收录或 Roll 数据异常；DIM 单独解释为命中组合数或同一条最接近组合的 `x/y Perk`，不得产生来源无关的共识或命中等级。
