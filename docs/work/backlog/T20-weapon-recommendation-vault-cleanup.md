...
> 更新时间：2026-09-02
> 状态：阶段 0–8 功能开发完成；待真实账号人工验收与五来源中文知识库再分发许可确认
> 优先级：P1
...
- 账号快照通过一次 Profile 请求生成紧凑 `weapon_roll`；SQLite 名称级来源输出六类明确要求，Core 按实例逐来源计算要求总数、符合数和逐栏状态，并通过 IPC/App 实例 Map 传输。仓库筛选卡、“目标与匹配”、同名整理和单件武器详情均消费 `source_matches`。
- DIM 原始 Hash 集合保持“无序且全部必需”语义；应用按当前武器官方 Socket 诊断唯一栏位、同栏多个必需项、跨栏歧义、缺失定义与特殊异域插槽。旧 Hash 只有在官方名称能唯一归属当前插槽时才映射，不猜测第三/第四栏。
- 资料库未准备、资料库版本落后和推荐知识库不可用使用结构化原因传输；统一 SQLite 仅存在 DIM 数据但中文推荐记录为空时仍视为中文知识库不可用，DIM 或本机规则继续保留部分结果。仓库提供只读验收报告，记录扫描完整性、Manifest 版本、推荐库 Schema/指纹、0/y、无法核对、异域、跨版本同名和 DIM 诊断。
...
T20 代码切片已经串行集成完成；剩余工作是使用真实账号执行只读人工验收，以及确认五来源中文知识库的公开再分发许可。
...
| 3 | ✅ 已完成 | 实现：SQLite 推荐库与迁移 | 是，服务层 | schema version 3 统一承接知识库、DIM 和本地社区推荐；旧 JSON 只保留一次性升级迁移入口，不参与运行时查询 |
...
| 6 | ✅ 代码完成，待真实账号实窗验收 | 实现：现有仓库与详情接入 | 是，共享 UI | 原筛选、目标和护甲保持不变；武器卡、实例来源列表、同名来源逐栏比较和单件详情六栏证据均已接入 |
...
阶段 0–8 的代码实施已经完成。阶段 3A 中 DIM 上游更新可用；中文知识库公开在线更新仍受五来源再分发许可约束，这属于发布前外部前置条件，不属于可由代码替代的开发切片。
...
- DIM 作者、标签、notes、标题和用途解析已经完成；完整实例 Plug 输入已经接通。UI 已在武器卡、“目标与匹配”、同名整理和单件详情正式消费实例结果，并将知识库 `x/y 项` 与 DIM `x/y 组` 分开显示；同名整理可在实例 Roll 与当前组来源要求间切换，单件详情按来源卡固定展示六栏要求、本件拥有、当前启用与文字状态。
...
- Core 已按“实例 × 来源 × 明确要求栏位”输出 `source_matches`：每个来源独立计算 `matched_requirement_count / requirement_count`，六栏固定返回符合、不符、来源未要求或无法核对，不跨来源拼接。
- 候选名称按官方名称精确解析，精确失败后只接受当前武器池内双向包含且唯一；未解析候选只有在与实例实际拥有的官方名称完全一致时才可确认符合，否则保留原文并标记无法核对。
- 批量实例结果不复制 DIM 等来源的超长注解和来源位置；单件推荐查询保留完整来源记录，详情现已按来源 ID 与实例匹配结果合并评级、说明、原始链接、版本、更新时间和原表位置。
...
1. `筛选列表`
   ...
   - 武器卡只显示玩家整理状态、保护事实和最多两条来源摘要（如 `Aegis 5/6`、`DIM 2/2组`）；知识库来源的分母是明确要求栏位，DIM 原生 Wishlist 的分母是组合数，两者不混用。不堆放逐槽位细节，不从来源摘要推导整理状态。
...
目标：提供只读来源对照，以及可撤销、可预览并受强制保护约束的玩家本地整理操作；系统不自动生成待处理结论，发布前仍需用真实仓库完成安全抽查。

已实施：

1. 玩家只能手动选择整理状态；来源对照不自动生成待处理。
...
5. “复制验收报告”提供真实账号只读抽查材料，覆盖五来源 0/y、无法核对、异域、跨版本同名、DIM 组合、扫描完整性和版本信息。
...
来源冲突按已确认事实判断：任一知识库来源或 DIM 规则已有符合项，而另一来源明确为 `0/y` 或 `0/全部组合` 时即触发保护；DIM 自身的部分符合不会同时被误算为负向证据。
