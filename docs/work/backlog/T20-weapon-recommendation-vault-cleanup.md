...
> 更新时间：2026-09-04
> 状态：✅ 阶段 0–12 已完成并通过真实账号、实窗与性能验收
> 优先级：P1
> 实施原则：CSV 维护、SQLite 查询、实例级证据、玩家确认后整理；应用不自动解锁或分解装备
> 验收记录：2026-09-03 用户确认验收完成；`pnpm ci:local` 同日通过文档、编码、构建、行为测试、质量门禁、架构测试、共享 Shell 视觉契约和全量类型检查
...
T20 代码切片已经串行集成，并完成真实账号四类位置、邮政官取回、推荐文案、仓库滚动性能和详情证据验收。五来源中文知识库公开再分发许可仍作为独立发布前置条件跟踪，不影响玩家本地导入使用。
...
| 4 | ✅ 已完成并验收 | 实现：完整仓库 Roll 扫描 | 是，账号服务 | 一次 Profile 请求得到每个武器实例实际拥有的全部可对照插槽，并生成紧凑 Roll 指纹与完整性状态 |
| 5 | ✅ 已完成 | 实现：Desktop IPC 与 App workspace | 是，接线层 | 实例结果、扫描状态和结构化依赖原因已贯通；失败保留旧结果，资料库落后时不输出伪精确结果 |
| 6 | ✅ 已完成并验收 | 实现：现有仓库与详情接入 | 是，共享 UI | 原筛选和护甲保持不变；仓库按“筛选列表 → 推荐与目标 → 同名整理”组织，推荐实例可直达详情证据和对应同名组，个人目标保留独立页签，数据管理使用焦点受限覆盖层 |
...
| 9 | ✅ 已完成并验收 | 实现：仓库缓存、虚拟网格与卡片重排 | 是，共享 UI 与服务层 | 匹配缓存、摘要索引和二维虚拟网格降低千件仓库重复计算；武器卡扩大并固定信息层级，护甲保持原逻辑 |
| 10 | ✅ 已完成并验收 | 实现：全账号武器位置与玩家匹配文案 | 是，Core/App/共享 UI/Desktop | 邮政官武器正确分类；仓库武器聚合所有位置并新增位置筛选；账号、仓库、邮政官按场景显示不同密度摘要，共用统一详情页和位置感知操作 |
| 11 | ✅ 已完成并验收 | 整理：仓库任务范围与详情渐进加载 | 是，App/共享 UI/Desktop | 默认进入仓库整理，随身待检查与全账号改为主动范围；账号快照 Roll 直接进入详情首屏，隐藏同名分组和非关键详情请求延后；推荐区自动打开非空来源 |
| 12 | ✅ 已完成并验收 | 整理：推荐匹配状态视觉层级 | 是，共享 UI | 全部符合、部分符合、未符合、仅推荐武器和数据不完整使用稳定的颜色、状态点与文字组合；仓库保持轻量，详情强化逐项证据，护甲不受影响 |

阶段 0–12 已完成并验收。阶段 3A 中 DIM 上游更新可用；中文知识库公开在线更新仍受五来源再分发许可约束，该事项保留为发布前外部前置条件，不影响 T20 本地导入、匹配、展示和整理主流程的完成状态。
...
### 6.15 补丁：弱网下 DIM 在线检查可恢复

验收后发现，DIM 在线检查在弱网或无法连接 GitHub 时，虽然服务层最终会超时，但原先把整个弹窗置于忙碌态，用户会长时间停留在“检查并读取中”。本补丁已完成：

1. 在线检查改为后台非阻塞；检查按钮单独显示“后台检查中”并禁止重复点击，其他导入、粘贴和查看入口保持可操作。
2. GitHub commit 检查超时为 30 秒，Wishlist 文件下载超时为 180 秒；失败时继续保留当前已启用数据。
3. 推荐数据弹窗增加 210 秒界面兜底超时，显示明确的网络超时和本地文件导入回退提示；在线检查阶段可点击“关闭”或按 Escape 离开弹窗。
4. 确认更新、本地导入和移除操作仍保持原有阻塞保护，不与后台检查共用全局忙碌锁。
4. 该处理只针对在线检查，不改变 SQLite 数据、推荐匹配结论或 DIM 本地导入格式。

完成门槛：断网、慢网和 GitHub 请求超时不会阻塞弹窗；检查期间可以继续使用其他入口或关闭窗口，检查完成后状态自动更新，当前推荐数据不被清空，重新打开仍可使用本地导入或再次检查。
