...
> 执行规则覆盖：本文保留的测试、`Red / Green / Tidy / Verify` 和 `verify:*` 命令仅是历史计划记录，不是当前 agent 执行要求。不得据此自动新增测试或执行旧命令；用户主动本地测试时照常运行现有测试。

> **给执行 agent：** 只把本文作为业务目标和架构参考，按仓库根目录 `AGENTS.md` 的实现、整理快路径推进；不要执行正文中的历史测试循环或验证命令。
