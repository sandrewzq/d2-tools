...
本任务只负责把攻略或自然语言目标转换成可核验要求，并结合真实账号给出匹配、缺口、替代方案和配装草稿。

- 仓库保留、复查和清理决策由“仓库推荐与清理工作台”负责；本任务只消费其已有规则或证据，不重复实现清理工作流。
- 商人当前库存、掉落来源和当前是否可刷由“商人覆盖与掉落来源”负责；本任务可以引用这些事实，但不维护日报或商人查询模型。
- AI 只负责语言理解和结果说明，不能替代 Bungie API、Manifest、账号实例、wishlist 或本地规则成为事实来源。
- 当前只做轻量护甲可达性和少量可解释方案，不扩展成完整 DIM 或 D2ArmorPicker 复刻。
