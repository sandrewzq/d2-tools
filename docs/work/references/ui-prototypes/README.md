...
按钮、字段、分段控件、缩略图和图标容器统一使用 `4px` 圆角；状态矩阵、摘要框与状态框使用 `4px` 轻圆角；独立对象卡使用 `6px`；Chip、进度与滚动条滑块使用胶囊圆角。页面、工作区、章节、目录和数据行保持直角。完整映射以 specs/global-visual-contract.md 的“圆角合同”为准。
