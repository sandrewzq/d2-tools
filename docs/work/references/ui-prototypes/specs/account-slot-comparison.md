...
装备对照是连续的满宽数据工作区：类别、位置标题和两列边界只使用直角 `PageSection`、`RowLine` 与 `SplitLine`。每件物品是独立 `ObjectCard`，拥有 `6px` 圆角和唯一对象边框；只有武器和护甲使用可点击对象卡并打开统一装备详情，装备类别与其他物品保持只读对象卡，不显示手型、hover 或详情入口。“显示全部”是 `secondary` Control，位置为空使用 `4px` 轻圆角 `state-frame`。不得把类别、位置或装备列再包成对象卡。
