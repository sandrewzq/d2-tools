...
| 活动奖励 | `entries[].rewards[]`、`onOpenWeeklyActivityReward` | 已确认且包含 Hash 的奖励可打开统一装备详情；不在首页伪造定义内容 |
...
| 活动奖励 | ready / missing | 已确认奖励显示详情入口并由应用读取当前 Manifest；缺失时显示“奖励待确认”，不伪造名称、图标或详情入口 |
