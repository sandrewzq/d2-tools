...
>
> 产品化：其中已在应用内展示的那部分站点清单落在 `packages/ui/src/directory/toolDirectory.ts`（工具导航菜单）。本文保留完整调研原貌，能力边界和交互参考结论仍以这里为准。
...
本节保存 T20“武器推荐汇总与仓库精确清理”的外部数据源调研。v0.0.22 已发布能力见 `CHANGELOG.md`，这里不重复维护完成状态。
