...
- `tools/git-auto-release.cmd`：维护者一键发布入口；自动把版本号 patch +1，例如 `0.0.10` 到 `0.0.11`；更新所有 package 版本和 `CHANGELOG.md`；运行 `check`、`test:docs` 和 `release:preview`；随后全量提交、push、创建并 push release tag。
