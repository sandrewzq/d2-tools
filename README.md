...
- 仓库完整列表，支持搜索、分组、排序、详情查看和本地标签。
- 物品详情弹窗，展示图标、类型、描述、perk/socket 信息。
- AI 助手页，支持本地确定性仓库分析，以及 OpenAI/DeepSeek/自定义兼容接口的深度建议。
- Windows x64 绿色包构建。

计划中：

- 装备详情增强，展示来源、perk 组合、收藏状态和推荐说明。
- AI 装备分析增强、perk 解读、配装建议和每周内容摘要。
...
- 点击“仓库”：查看完整仓库列表，并给装备标记“保留”“关注”或“可清理”。
- 在“物品搜索”里输入装备中文名或英文名，例如 `风险管理者` 或 `Riskrunner`。
- 点击装备卡片或搜索结果，查看物品详情和 perk 信息。
- 点击“AI 助手”：先做本地分析；如果配置了 AI，再生成深度建议。
- 点击“去配置”：可以重新查看和修改之前保存的 Bungie 配置。

### 可选：配置 AI 助手

AI 功能不是必填项。不配置 AI 时，d2-service 仍然可以登录 Bungie、读取账号、搜索物品、查看仓库和做本地分析。

如果你要使用 AI 深度建议：

1. 打开左侧“AI 助手”。
2. 在“AI 提供方”里选择：
   - `DeepSeek`：模型可填 `deepseek-chat`，接口地址可留空。
   - `OpenAI 兼容接口`：模型按你的账号可用模型填写，接口地址可留空。
   - `自定义兼容接口`：适合本地模型或第三方兼容服务，需要填写接口地址。
3. 填写自己的 `API Key`。
4. 填写模型名称。
5. 如果是自定义接口，在“接口地址”填写类似：

```text
http://127.0.0.1:11434/v1
```

6. 点击“保存并测试连接”。如果提示连接成功，说明 API Key、模型和接口地址可以使用。
7. 下方点击“AI 深度建议”。

注意：

- AI 配置只保存在本机，不会打包进程序。
- AI 分析会把当前仓库摘要、实际 roll 和本地标签发给你配置的 AI 服务。
- 如果你不想把装备数据发给第三方模型，只使用“本地分析”即可。
