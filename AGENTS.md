...
- 对用户的最终回答、本次改动里新增或重写的仓库文档，成稿前调用 `shuorenhua` skill 过一遍。skill 真源在 `~/.agents/skills/shuorenhua/`，Claude Code 里是 `/shuorenhua`。不能只凭主观判断说「已经够直白」就跳过这一步。
- 覆盖范围：`docs/**`（含 backlog、合同文档、`todo.md`、`development.md`）、`README.md`、`AGENTS.md`、CHANGELOG 的中文条目，以及面向用户的回答和状态说明。一两句的即时状态说明不单独调用，但按同一口径写。
- 不覆盖：代码、代码注释、代码围栏内的文本、命令、路径和 hash。skill 自身也会保护代码块，不要借这次改写去动注释或程序行为。
- 口径不放宽：数字、版本、`T数字` / `Bug #数字`、`file:line`、术语原名、责任主体（谁做什么、何时、什么条件、多大范围、否定与完成状态）和「未运行本地自动化验证」这类限定语必须原样保留。skill 的职责是删包装语、客套和翻译腔，不是概括、重排或改判据；它主张删的内容若受本文件保护，以本文件为准。
- 结构不进入改写：标题层级、表格列、编号、链接目标、CHANGELOG 的 `### 中文` / `### English` 段结构与版本号保持原样，只在原位改文字。
- 只要问题清单时用只标问题模式（`annotation mode`），不要顺手交出改写稿。
...
- 仓库根的 `CLAUDE.md` 只做入口指针（`@AGENTS.md`），让 Claude Code 能加载到本文件；规则内容不在那里写第二份，改动一律回到本文件。
